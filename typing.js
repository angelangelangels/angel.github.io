// <![CDATA[
var news = Array(
  "hiii! o(≧▽≦)o i'm angel. welcome to angel city. enjoy your stay and thank you for stopping by."
);
...
function teleprint() {
  if (document.getElementById) {
    ...
    delay *= 4000; //default 1000
    newsp = document.createElement("span");
    cursp = document.createElement("span");
    cursp.appendChild(
      document.createTextNode(String.fromCharCode(160) + cursor)
    );
...
function ticker() {
  ...
  for (i = 1; i < news[item].length; i++)
    setTimeout(
      'newsp.firstChild.nodeValue="' + news[item].substring(0, i + 1) + '"',
      100 * i
    );
  if (news[item].indexOf("www") != -1)
    setTimeout("linkit(" + item + ")", 100 * i);
  setTimeout(
    "flash=setInterval(\"cursp.style.visibility=(cursp.style.visibility=='visible')?'hidden':'visible'\", 444)",
    100 * i
  ); //default 234
